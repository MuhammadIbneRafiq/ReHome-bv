...
export const EnhancedDatePicker: React.FC<EnhancedDatePickerProps> = ({ 
        ...
        const calculateDayInfos = async () => {
            ...
            try {
                for (let day = 1; day <= daysInMonth; day++) {
                    ...
                    const [cityStatus, allCitiesEmpty] = await Promise.all([
                        getCityScheduleStatus(pickupCity, date, window.location.origin),
                        checkAllCitiesEmpty(date, window.location.origin)
                    ]);
                    ...
                    if (pickupCity === dropoffCity) {
                        ...
                        if (isCityDay || isEmpty) {
                            basePrice = (cityBaseCharges[pickupCity]?.cityDay || 0 + cityBaseCharges[dropoffCity]?.normal || 0) / 2;
                ...
                console.error('Error calculating day infos:', error);
            } finally {
                setIsLoading(false);
            }
    ...
    }, [currentYear, currentMonth, pickupPlace, dropoffPlace]);
            ...
            <PopoverContent align="start" className="w-96 p-4">
                <div className="space-y-4">
                    <div className="flex items-center justify-between">
                        <button onClick={goPrev} className="p-1 hover:bg-gray-100 rounded" type="button" aria-label="Previous month">
                            ◀
                        </button>
                        <h3 className="font-semibold">
                            {monthNames[currentMonth]} {currentYear}
                        </h3>
                        <button onClick={goNext} className="p-1 hover:bg-gray-100 rounded" type="button" aria-label="Next month">
                            ▶
                        </button>
                    </div>
                        ...
                        {cells.map((c, idx) => {
                            ...
                            return (
                                <button
                                    ...
                                    disabled={isLoading}
                                    className={`p-2 text-sm rounded-full text-center relative transition-colors
                                        ${isSelected ? 'bg-blue-500 text-white' : 
                                          dayInfo?.isGreen ? 'bg-green-100 text-green-800 hover:bg-green-200' : 
                                          'hover:bg-gray-100'}
                                        ${isLoading ? 'opacity-50 cursor-not-allowed' : ''}`}
                                    title={dayInfo ? `${dayInfo.priceType}: €${dayInfo.basePrice}` : 'Loading...'}
                                >
                                    {c.day}
                                    {dayInfo?.isCityDay && (
